...
# Upload Images Page
    ...
    if "uploaded_files_by_series" not in st.session_state or not st.session_state.uploaded_files_by_series:
        ...
        if "results_df" in st.session_state:
            ...
            # Dropdown to select row for visualization
            st.markdown("### Select a Prediction to Visualize")
            selected_row_id = st.selectbox(
                "Select a row ID:",
                options=results_df['row_id'],
                help="Choose a specific prediction to view probabilities."
            )

            # Display probability values and bar chart
            selected_data = results_df[results_df['row_id'] == selected_row_id].iloc[0]
            st.markdown(f"### Probability Values for {selected_row_id}")
            st.write(f"- **Normal/Mild:** {selected_data['normal_mild']:.2f}")
            st.write(f"- **Moderate:** {selected_data['moderate']:.2f}")
            st.write(f"- **Severe:** {selected_data['severe']:.2f}")

            # Bar chart visualization
            chart_data = pd.DataFrame({
                'Probability': ['Normal/Mild', 'Moderate', 'Severe'],
                'Value': [
                    selected_data['normal_mild'],
                    selected_data['moderate'],
                    selected_data['severe']
                ]
            })
            st.markdown(f"### Probability Distribution")
            st.bar_chart(chart_data.set_index('Probability'))

        # Clear All Uploaded Files
        if st.button("Clear All Uploaded Files"):
            st.session_state.uploaded_files_by_series = {}
            st.session_state.results_df = None
            st.success("All uploaded files and predictions have been cleared.")
